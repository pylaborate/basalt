...
def test_setvars():
    ...
    mkv.define(
        ...
        env_bindir=lambda: venv_bindir(mkv.venv_dir),
    ...
    print("! TEST mkvars keys => " + repr(tuple(v for v in mkv.keys())))
    ## each of these may cause expansion by side effect:
    # print("! TEST mkvars values => " + repr(tuple(v for v in mkv.values())))
    # print("! TEST mkvars items => " + repr(tuple((k, v,) for k, v in mkv.items())))

    if do_mkdup_test():
        mkv.eval()  # this prevents the initial mkdup tests from failing ...
        mkdup = mkv.dup()
    ...
    if "TEST_MKDUP_POST" in os.environ:
        postdup = mkv.dup()
        assert_that(postdup.stampdir).is_equal_to("{build_dir}/.build_stamp".format_map(mkv))
        assert_that(postdup.value(postdup.stampdir)).is_equal_to("{build_dir}/.build_stamp".format_map(mkv))
        assert_that(postdup['stampdir']).is_equal_to("{build_dir}/.build_stamp".format_map(mkv))
        assert_that(postdup.requirements_depends).contains("requirements.in")
        assert_that(postdup['requirements_depends']).contains("requirements.in")
        assert_that(postdup.mapped_dct.keys()).contains("a", "b")
        for value in postdup.mapped_dct.values():
            assert_that(isinstance(value, str)).is_true()
            assert_that("{" in value).is_false()
        assert_that(postdup.mapped_dct['a']).is_equal_to("{pip_cache}".format_map(postdup))
        assert_that(postdup.mapped_dct['b']).is_equal_to("{project_py}".format_map(postdup))
        postdup.reset() # !
        assert_that(postdup.value(postdup.stampdir)).is_equal_to("{build_dir}/.build_stamp".format(**mkv))
        assert_that(postdup['stampdir']).is_equal_to("{build_dir}/.build_stamp".format(**mkv))
        assert_that(postdup.requirements_depends).contains("requirements.in")
        assert_that(postdup['requirements_depends']).contains("requirements.in")
